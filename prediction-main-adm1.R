...
#                             ( age_1524 * female )
model.formula <- response ~ 1 + (age_1524 * female) + (sindh + age_65up : sindh) + (kpk + underweight_2012 : kpk)
...
###############################################################################
## Clip predictions onto the Alhasan province shapes

OFFICIAL.SHAPE.FILE.NAME <- file.path("data", "shapefiles", "alhasan-pk-adm-1-with-clusters.shp")
...
# Print the results
shape.adm2@data[, c("DOT_NAME", "pred.prev.mean", "pred.prev.std")]
